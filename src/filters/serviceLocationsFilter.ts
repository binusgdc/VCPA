        ...
        return {
            async handle(command) {
                ...
                if (serviceLocation === undefined) {
                    console.log(
                        `>>> ${command.user.id} tried to issue commands from without being in a serviced guild!`
                    )
                    await command.reply(
                        `<@${command.user.id}> tried to issue commands without being in a serviced guild!`
                    )
                    return
                }
                ...
                if (!executorAsMember.roles.cache.hasAny(...serviceLocation.commandAccessRoleIds)) {
                    console.log(
                        `>>> ${command.user.id} tried to issue commands without having the appropriate permission!`
                    )
                    await command.reply(
                        `<@${command.user.id}> tried to issue commands without having the appropriate permission!`
                    )
