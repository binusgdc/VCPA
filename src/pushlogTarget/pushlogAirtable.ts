	...
	public async push(logData: PushlogData): Promise<PushlogResponse> {
		try {
			...
			void this.logger.debug(
				`Starting attendance record processing. Batch size: ${this.createBatchSize}. Batches: ${Math.ceil(
					logData.attendees.length / this.createBatchSize
				)}`
			);
			...
			for (let i = 0; i < logData.attendees.length; i += this.createBatchSize) {
					...
					if (finishedResult.status === "rejected") {
						void this.logger.fatal(
							`Unexpected error in finding member: "${
								finishedResult.reason instanceof Error ? finishedResult.reason.message : "UNKNOWN ERROR"
							}". Continuing...`
						);
						continue;
					}
				...
				if (batch.length - attendanceBatchPayload.length === 0)
					...
					void this.logger.warn(
						`Completed batch insertion with ${batch.length - attendanceBatchPayload.length} failed entries.`
					);
				}
			}
