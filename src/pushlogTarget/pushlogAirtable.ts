    ...
    public async push(logData: PushlogData): Promise<PushlogResponse> {
        try {
            ...
            const sessionRecordPayload = [
                {
                    fields: {
                        ...
                        "Session Duration": Math.max(
                            Math.trunc(logData.sessionDuration.as("seconds")),
                            60
                        ),
                        "Recorder (Name String)": logData.recorderName,
                        "Mentor (Discord UID)":
                            logData.mentorDiscordUserIds.length !== 0
                                ? logData.mentorDiscordUserIds.reduce(
                                      (prev, next) => `${prev}, ${next}`
                                  )
                                : "",
                    },
            ...
            void this.logger.debug(
                `Creating session record in base, payload: ${sessionRecordPayload.toString()}`
            )
            ...
            void this.logger.debug(
                ...
                }. Batches: ${Math.ceil(logData.attendees.length / this.createBatchSize)}`
            )
            ...
            for (let i = 0; i < logData.attendees.length; i += this.createBatchSize) {
                ...
                const memberRecordsResults = await Promise.allSettled(
                    batch.map(
                        async (
                            attendance: AttendanceDetail
                        ): Promise<[AttendanceDetail, MemberDetailRecord | null]> => {
                            const memberResult = await this.base(this.membersTableId)
                                .select({
                            ...
                            const [recordId, nameResult, nimResult] = [
                                memberResult[0].id,
                                memberResult[0].get("Name"),
                                memberResult[0].get("NIM"),
                            ]
                            ...
                            try {
                                ...
                                throw new Error(
                                    `Unexpected result from airtable. Fields "Name" and "NIM" are expected to be interpretable as strings. Instead got: Name - ${typeof nameResult}, NIM - ${typeof nimResult}`
                                )
                            }
                            return [
                                attendance,
                ...
                for (const finishedResult of memberRecordsResults) {
                    if (finishedResult.status === "rejected") {
                        void this.logger.fatal(
                            `Unexpected error in finding member: "${
                                finishedResult.reason instanceof Error
                                    ? finishedResult.reason.message
                                    : "UNKNOWN ERROR"
                            }". Continuing...`
                        )
                        continue
                    ...
                    if (memberDetails === null) {
                        void this.logger.error(
                            `Discord ID <@${attendance.discordUserId}> not found in members`
                        )
                        continue
                    }
                    void this.logger.info(
                        `Found member: ${memberDetails.name} ${memberDetails.nim}`
                    )
                    attendanceBatchPayload.push({
                        fields: {
                            "Session ID": [sessionRecordId],
                            "Student ID": [memberDetails.recordId],
                            "Attend Duration": Math.max(
                                Math.trunc(attendance.attendanceDuration.as("seconds")),
                                60
                            ),
                        },
                    })
