...
module.exports = {
	...
	exec: (msg) => {
		...
		if(ms <= 0 || ms >= Number.MAX_SAFE_INTEGER){
			return global.clientChannel.send("Invalid time value")
		}
		...
		for (let i = 0; i < global.maxSessionCount; i++) {
			if (global.sessions[i] === undefined) {
				...
				global.clientChannel.send(`<@${msg.author.id}> started a session in <#${msg.guild.members.cache.get(target).voice.channelId}>!`);
				...
				global.clientChannel.send(`Session will stop automatically after ${time} minute(s)`)
				break;
			} else {
