...
function generateStartSessionRequest(n_members?: number): {
    ownerId: Snowflake
...
test("startSessionInChannelWithOngoingSessionFails", async () => {
    ...
    const sut = new SessionService(
        instance(ongoingSessionStoreMock),
        instance(sessionLogStoreMock),
        instance(dateTimeProviderMock)
    )
...
test("startSessionStoresOngoingSessionWithInitialJoinEvents", async () => {
    ...
    const expected = members.map<SessionEvent>((member) => ({
        type: "Join",
        userId: member,
        timeOccurred: now,
    }))
